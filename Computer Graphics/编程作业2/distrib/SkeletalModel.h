...
public:
	...
	void drawJoints( );
	void dfsJoint(Joint *joint, MatrixStack *stack);

	// 1.2. Implement this method a recursive helper to draw a box between each pair of joints
	void drawSkeleton( );
	void dfsSkeleton(Joint *joint, MatrixStack *stack);
	...
	void dfscomputeBindWorldToJointTransforms(Joint *joint, MatrixStack *stack);

	// 2.3.2. Implement this method to compute a per-joint transform from
	// joint space to world space in the CURRENT POSE.
	void updateCurrentJointToWorldTransforms();
	void dfsupdateCurrentJointToWorldTransforms(Joint *joint, MatrixStack *stack);
